        ...
        <div class="row">
            <div class="col-md-12 col-xs-12">
                <div class="panel panel-default">
                    <div class="panel-heading">Ubah Data RAP</div>
                    ...
                    <input type="hidden" id="urut" value='0'>
                        ...
                        @endif
                         {!! Form::open(['url' => ['/raps', $rap->nonota ], 'method' => 'patch', 'class' => 'form-horizontal', 'files' => true]) !!}
                                {{csrf_field()}}
                                ...
                                <table width="100%">
                                    <tr height="50px">
                                        ...
                                        <td width="200px"><select class='kodeKelompokKegiatan form-control' name='pekerjaan' id='pekerjaan"+itung+"' onchange='setsatuan(this)' required>
                                                <option value=""> -- Pilih Pekerjaan -- </option>
                                                        @foreach ($pekerjaans as $pekerjaan)
                                                            @if($kerja->kodeKelompokKegiatan==$pekerjaan->kodeKelompokKegiatan)
                                                            <option value='{{ $pekerjaan->kodeKelompokKegiatan }}' selected> {{ $pekerjaan->kodeKelompokKegiatan }} - {{$pekerjaan -> nama}}  </option>
                                                            @else
                                                           <option value='{{ $pekerjaan->kodeKelompokKegiatan }}'> {{ $pekerjaan->kodeKelompokKegiatan }} - {{$pekerjaan -> nama}}  </option>
                                                           @endif
                                                        @endforeach
                                                    </select>
                                                    <input type="hidden" name="kerja" value="{{$kerja->kodeKelompokKegiatan}}">
                                        </td>
                                    </tr>
                                    <tr height="50px">
                                        <td width="100px" align="right" style="padding-right:10px; "> Tanggal RAP </td>
                                        <td width="200px"><input class="form-control date-picker birthday" type="text" required name="tglNota" value="{{date('m/d/Y',strtotime($rap->tglNota))}}" ></td>
                                        <td width="100px" align="right"  style="padding-right:10px;">Keterangan</td>
                                        <td width="200px">
                                            <input type="text" name="keterangan" class="form-control" value="{{$kerja->keterangan}}">
                                        </td>
                                    </tr>
                                    <tr height="50px">
                                ...
                                <div class="table table-responsive">
                                      <table class="table">
                                       ...
                                       <input type="hidden" name="no" id="no" value="{{$detail->count()}}">                                  
                                        <tbody id='detail'>
                                             @php
                                            $itung=1;
                                            $stn='';
                                            ...
                                            <tr id='no{{$itung}}'>
                                                ...
                                                <option value=""> -- Pilih Kegiatan -- </option>
                                                        @foreach ($kegiatans as $kegiatan)
                                                            @if($det->kodeKelompokKegiatan==$kegiatan->kodeKelompokKegiatan)
                                                           <option value='{{ $kegiatan->kodeKelompokKegiatan }}' selected> {{ $kegiatan->kodeKelompokKegiatan }} - {{$kegiatan -> nama}}  </option>
                                                           @php
                                                           $stn=$kegiatan->satuan;
                                                           @endphp
                                                            @else
                                                ...
                                                <td width='' align='right' style='padding-right:10px;'><input type='number' min='1' name='minggu_mulai[]' class='minggu_mulai form-control' id='minggu_mulai{{$itung}}' value='{{$det->minggu_mulai}}' ></td>
                                                <td width=''><input type='number' name='lama[]' class='lama form-control' id='lama{{$itung}}'  min='1' value='{{$det->lama}}'  onkeyup='getTotal({{$itung}})' onchange='getTotal({{$itung}})' ></td>
                                                <td width=''><input type='number' value="{{$det->volume}}" min='0' step='any' name='volume[]' id='volume{{$itung}}' class='form-control volume' onkeyup='getTotal({{$itung}})' onchange='getTotal({{$itung}})' ></td>
                                                <td width='' id='satuan{{$itung}}' >{{$stn}}</td>
                                                <td width=''><input type='number' step='any' name='hargaSat[]' id='hargaSat{{$itung}}' class='harsat form-control' value="{{$det->totalHarga/$det->volume}}" readonly></td>
                                                <td width=''><input type='number' value="{{$det->totalHarga}}" step='any' name='total[]' id='total{{$itung}}' class='form-control total' readonly></td>
                                        ...
                                        <tfoot>
                                             <tr>
                                                ...
                                                <td colspan="2">
                                                    <b><input type="text" class='form-control' readonly value='{{$grand}}' name="grandTot" id="grandTot"></b>
                                                </td>
                                                <td></td>
                                            </tr>
                                            <tr>
...
$(document).ready( function ()
{
    $('#tambah').click(function(e){
        ...
        var baris = "<tr id='no"+itung+"'>"+
                        "<td width='20%'>"+
                        "<select class='kodeKelompokKegiatan form-control' name='kodeKelompokKegiatan[]' id='kodeKelompokKegiatan"+itung+"' onchange='setsatuan(this)'>"+
